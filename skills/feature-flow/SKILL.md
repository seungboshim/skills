...
> 이 문서화 절차는 **`worklog` 스킬로 독립 추출**돼 있다 — 사이클 밖에서 단독 호출 시 `/worklog`. 아래는 동일 절차의 인라인 버전이며, **backlog 갱신**(끝낸 항목 체크 + 신규 후속 추가)도 함께 한다.
...
- 사이클의 **입구**는 `daily`(오리엔트+작업 추천), **출구**는 `worklog`(패치노트+사양+backlog). feature-flow 는 그 사이 전체를 끌고 가는 풀버전이고, 가볍게 갈 땐 `daily` → 작업 → `worklog` 만으로도 굴러간다.
